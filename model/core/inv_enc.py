...
class INV_ENC(nn.Module):
    ...
    def forward(self, X, L=1, train=True):
        ''' 
            X is [N,T,nc,d,d] or [N,T,q] 
            returns [L,N,T,q]
        '''
        c = self.inv_encoder(X, train=train) # N,Tinv,q
        if self.is_last_layer_gp:
...
class InvariantEncoderCNN(EncoderCNN):
    ...
    def forward(self,X,train=True):
        ...
        T_inv = min(T_inv,T)
        if train:
            #randomly sample subsequences for invariance 
            ...
            #take last T_inv frames before the prediction 
            X = X[:,-T_inv:] 
            X = X.reshape(T_inv * N, nc, d,d)
...
class InvariantEncoderRNN(EncoderRNN):
    def __init__(self, input_dim, T_inv=None, rnn_hidden=10, enc_out_dim=16, out_distr='dirac'):
        ...
        self.T_inv = T_inv
    def forward(self, X, ns=5, train=True):
        [N,T,d] = X.shape
        T_inv = T//2 if self.T_inv is None else self.T_inv
        T_inv = min(T_inv,T)
        if train:
